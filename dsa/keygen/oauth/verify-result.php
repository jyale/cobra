...
<? 
	...
	// only make oauth token if signature correctly verified
	if(count($pieces2) >= 2){
	
	// Make OAuth token and save to file
	$oauth_token = sha1(time());
	echo("OAuth token: " . $oauth_token);
	file_put_contents('oauth-tokens/' . $oauth_token, $userlogin);
	// redirect back to redirect_uri with oauth token as url parameter (get)

	$_REDIRURL = $_REQUEST['redirect_uri'] . '?access_token=' . $oauth_token;	
