...
for i in range(n):
	# private keys
	response = urllib2.urlopen('http://mahan.webfactional.com/dsa/keygen/server' + str(i) + '/getpriv.php?id=' + str(id))
	html = response.read()
	privs[i] = int(html)
	# public keys
...
def getpub(id):
	...
	for i in range(n):
		# public keys
		response = urllib2.urlopen('http://mahan.webfactional.com/dsa/keygen/server' + str(i) + '/getpub.php?id=' + str(id))
		html = response.read()
		pubparts[i] = int(html)
