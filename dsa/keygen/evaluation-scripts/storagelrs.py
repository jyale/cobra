...
#		print str(i) + ',' + str(j) + ' ' + str(verify(sign(m,L,X[i],j),m,L))


# print verify(sign(m,L,X[0],0),m,L)

# save the signature
f = open('files/' + sys.argv[1],'w')
pickle.dump((sign(m,L,X[0],0),m),f)
f.close()

# save the keys
f = open('files/' + sys.argv[1] + '.keys','w')
pickle.dump((L),f)
f.close()

#f2 = open('weak','r')

#sig,m2,L2 = pickle.load(f2)
#print verify(sig,m2,L2)
